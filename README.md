...
Audio playback relies on [gopxl/beep](https://github.com/gopxl/beep) and [ebitengine/oto](https://github.com/ebitengine/oto):

| Platform | `CGO_ENABLED=1`                          | `CGO_ENABLED=0`     |
|----------|------------------------------------------|---------------------|
| Linux    | 🔊 sound (needs ALSA, `libasound2-dev`)  | 🔇 silent fallback  |
| macOS    | 🔊 sound                                 | 🔇 silent fallback  |
| Windows  | 🔊 sound                                 | 🔊 sound (pure Go)  |

Binaries built with `CGO_ENABLED=0` still work everywhere: the URL checks run
normally, only the cheering jingle is skipped (a warning is logged instead).
